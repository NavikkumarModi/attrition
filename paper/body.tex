...
declines it, incurring instantaneous private regret $\epsilon$. Summing over the
$m$ deferrals gives $m\epsilon$. Any policy that instead pulls $H$ early forfeits
...
$m \in \{2,4,8,12\}$ and $\delta E \in \{0.5,1.5\}$: private regret of the optimal
policy is $m\epsilon$ to machine precision while its system-value advantage over
greedy is $m\delta E$.
...
statement---a policy that preserves value \emph{must} report regret. Being unable to
tell the two apart is not an oversight in how the metric is read; the metric orders
them backwards.

\begin{corollary}[The dashboard reading tracks the benefit]
\label{cor:tracks}
...
independent of $m$. The worse a value-preserving policy appears on the standard
metric, the more value it is delivering.
\end{corollary}

Verified exactly: sweeping $\epsilon \in \{0.01,0.05,0.10,0.20,0.40\}$ at $m=8$,
$\delta E = 1$, the system gain is $8.0000$ throughout while private regret runs
$0.08, 0.40, 0.80, 1.60, 3.20$ --- ratios of exactly $\epsilon$.

\paragraph{A second benchmark.} Because the choice of benchmark is doing the work,
we report both. Define \emph{system-value regret} $R_{\mathrm{sys}}(\pi) = V^\star -
W(\pi)$ against the ex-ante optimal policy. On random instances ($n=6$, $T=8$, 15
instances, exact DP):
...
The two regret columns rank the three policies in opposite orders. This is the
cleanest statement of the paper's point: the algorithms are fixed and the verdict is
determined entirely by which benchmark is consulted.
...
comparison. This removes the need for a coupling argument, and with it the
horizon-truncation difficulty that obstructed our earlier attempt: the identity
holds over the realised episode however long it turns out to be.
...
product breaks it (10/10, gap $5.4$--$6.3\%$). Separately, the interchange inequality
$v_a - v_b \ge p_a D_a - p_b D_b$---now a corollary of the theorem rather than a
step toward it---was audited exhaustively: $0$ violations in $6{,}842$ ordered
...
Two findings deserve emphasis, one supporting and one qualifying.
...
\emph{The ratio heuristic is competitive at this scale}, within $1\%$ of ECI. We
report this rather than only the greedy comparison. Against exact optimum at $n=6$,
however, ECI dominates it at every setting tested---gaps of $0.14$--$2.31\%$ versus
ratio's $1.27$--$15.77\%$---and the margin widens with the horizon, exactly as the
$(T-t)$ term predicts. Our reading is that with many arms a good low-$\kappa$
substitute is nearly always available, so the fine-grained weighting matters less;
ECI's derivation, not its empirical margin at large $n$, is the contribution.
