...
\paragraph{A natural lemma is false.} The first attempt conjectured
$V(S',t) \ge V(S,t)$ whenever $S' \supseteq S$ --- having more available arms is
never worse, since a policy can simply ignore the extra ones. Exhaustive checking
falsifies this: $904$ violations in $11{,}904$ triples tested, worst case
...
\paragraph{A residual shortfall at $n=40$, diagnosed and resolved.} A first pass
at larger scale showed SECI beating plain ECI everywhere but trailing greedy by a
small margin in the mid-$q$ range. Isolating the cause rather than accepting it:
varying cluster size and count independently left the gap unchanged, ruling out
cluster structure; sweeping $n$ directly showed both policies' values turning
\emph{negative} once $n$ exceeded roughly $20$ under a $\delta$ held fixed across
the sweep. The cause was an unfair comparison, not a failure of the correction:
...
\paragraph{Code and reproducibility.} The library, released as \textsc{Attrition} (Agent Testbed for Task, Resource and Irreversible-Tradeoff Investigation), all 48 experiments including
negative results, and one regression test per theorem are released openly.
