...
\begin{proof}
Write the total value of a policy $\pi$ over its realised episode as
\[
W(\pi) \;=\; \mathbb{E}_\pi\Big[\textstyle\sum_{t} v_{a_t}\Big] \;-\; \mathbb{E}_\pi\Big[\textstyle\sum_{t} B(S_t)\Big],
\]
and consider the burden term. A destruction at round $s$ imposes $\delta e_{a_s}$
on every later round of the episode, so
\[
\mathbb{E}_\pi\Big[\textstyle\sum_t B(S_t)\Big]
\;=\; \delta\, \mathbb{E}_\pi\Big[\textstyle\sum_{s} \mathbf{1}[\text{destruction at } s]\; e_{a_s} \,(N-s)\Big],
\]
where $N$ is the episode length. Conditioning on the history and the arm chosen at
round $s$, the destruction indicator has mean $p_{a_s}$, so round $s$ contributes
$p_{a_s} e_{a_s} = \kappa_{a_s}$ in expectation. Under the hypothesis
$\kappa_a \equiv \kappa$ this is the same constant \emph{whichever arm the policy
selects}, giving
\[
\mathbb{E}_\pi\Big[\textstyle\sum_t B(S_t)\Big] \;=\; \delta\kappa\, \mathbb{E}\Big[\textstyle\sum_{s}(N-s)\Big]
\;=\; \delta\kappa\, \mathbb{E}\!\left[\tfrac{N(N-1)}{2}\right].
\]
It remains to note that $N$ is itself policy-independent. Arm $j$ absorbs exactly
$G_j \sim \mathrm{Geometric}(p_j)$ pulls before destruction, and every pull is
allocated to exactly one arm, so the number of pulls until the pool is exhausted is
$N_{\mathrm{exh}} = \sum_j G_j$ \emph{regardless of the order in which a policy
allocates them}. Hence $N = \min(T, N_{\mathrm{exh}})$ has a policy-independent
distribution.

The burden term is therefore a constant common to all policies, so
\[
\arg\max_\pi W(\pi) \;=\; \arg\max_\pi \mathbb{E}_\pi\Big[\textstyle\sum_t v_{a_t}\Big],
\]
which is the same problem with no externality at all ($e \equiv 0$). There
destruction is geometric, hence memoryless, and rewards are stationary, so an arm's
total expected yield $v_a/p_a$ is independent of when harvesting begins: deferring
a pull buys no option value and costs the per-round gap. Greedy is optimal in that
problem, and therefore in this one, at every state and every horizon.

\emph{Necessity.} The construction of Theorem~\ref{thm:vacuity} has
$\kappa_H = E > 0 = \kappa_S$ and gap exactly $m\delta E > 0$, exhibiting the
required instance. \qed
\end{proof}

The step that closes the argument is that constant $\kappa$ makes the \emph{entire}
expected burden policy-invariant, not merely the per-arm burden term in the Bellman
comparison. No coupling argument is needed, and horizon truncation
presents no difficulty: the identity holds over the realised episode however long it
turns out to be.

\paragraph{Exact verification of the invariance.} We confirm the key identity
directly by dynamic programming, computing $\mathbb{E}[\sum_t B(S_t)]$ exactly for
...
product breaks it (10/10, gap $5.4$--$6.3\%$). The theorem also implies the interchange inequality
$v_a - v_b \ge p_a D_a - p_b D_b$, which we audit exhaustively as a consistency
check: $0$ violations in $6{,}842$ ordered pairs under constant $\kappa$, versus
$56.2\%$ when $\kappa$ varies.
