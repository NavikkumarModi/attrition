...
optimum throughout, where plain ECI's gap grows to roughly $11\%$ at $q=1$. At
larger scale ($n=40$, eight clusters, Monte Carlo evaluation since exact DP is
infeasible) SECI reliably beats plain ECI at every $q$ tested, but shows a small
residual shortfall against greedy in the mid-$q$ range that a cluster-size-scaled
variant of the exponent did not close. We report this as a genuine partial
result: the correlated-destruction failure mode is substantially fixed, and the
fix is never worse than the problem it replaces, but it does not yet carry the
small-scale guarantee to arbitrary scale.
...
only the additional cluster shock rate $q$. The small-scale guarantee is exact;
the large-scale guarantee is empirical and not yet complete.
\end{corollary}
