...
\paragraph{Results.} Figure~\ref{fig:domains} runs all four. Greedy records
cumulative regret of exactly $0.0000$ in every domain, and is beaten in every
domain---by $30$--$49\%$ where $\kappa$ dispersion is largest. Thompson sampling,
...
three of the four domains---the practical form of Theorem~\ref{thm:floor}.

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{fig6_domains.pdf}
\caption{Four domains, 30 seeds each. \textbf{(a)} Value relative to greedy.
Correction pays everywhere, most where $\kappa$ dispersion is largest.
...
loss under every one ($0.44$--$1.82$). This follows from the proof: zero regret is a
...
for saturating ($0.135\%$) and concave ($0.709\%$). Measuring the marginal burden of
...
residual gap under non-separable coupling is $0.14\%$--$0.71\%$, against the
$5\%$--$12\%$ gap greedy incurs when $\kappa$ genuinely varies.
